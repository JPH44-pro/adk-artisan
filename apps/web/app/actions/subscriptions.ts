 ...
 * to support subscription cancellation flows with after_completion
 */
      ...
      flow_data: {
        ...
        after_completion: {
          ...
          redirect: {
            return_url: `${
              process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"
            }/profile?message=cancelled`, // Success-only URL
          },
        },
      },
    };
...
export async function createCheckoutSession() {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price: STRIPE_CONFIG.PAID_PRICE_ID,
          quantity: 1,
        },
      ],
      success_url: `${
...
export async function getCustomerPortalUrl(): Promise<{
  ...
  if (result.success && result.portalUrl) {
    ...
    return { success: true, url: result.fallbackUrl };
  } else {
    return {
      success: false,
      error: "Portal unavailable",
      url: "/profile?error=portal_unavailable",
    };
  }
}
...
/**
 * Create checkout session for paid plan ($9.99/month)
 */
export async function createPaidCheckoutSession(): Promise<void> {
