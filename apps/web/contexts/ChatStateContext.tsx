...
export interface ChatStateContextType {
  ...
  // Usage and limits (derived from context)
...
export function ChatStateProvider({
  ...
  const limits = useChatLimits({ session });
  ...
  const messageFlow = useChatMessageFlow({
    ...
    isMessageLimitReached: limits.isMessageLimitReached,
    usageLoading,
  });
  ...
  }, [session?.id, polling.startPolling]);
  ...
  const contextValue: ChatStateContextType = {
    ...
    // Usage and limits
    isInputBlocked: limits.isInputBlocked,
    isMessageLimitReached: limits.isMessageLimitReached,
    isSessionLimitReached: limits.isSessionLimitReached,
